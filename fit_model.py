...
model.fit(train_features, val_features=test_features)
predictions = model.predict(test_features)
metrics = predictions.metrics_df
print(metrics)
metrics.to_csv(f"{model.__class__.__name__}.csv")
model.save_model(f"trained_models/{model.__class__.__name__}")